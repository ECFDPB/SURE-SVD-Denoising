function [denoised, psnr_val, ssim_val] = energy_matching_svd_denoising(noisy, sigma, clean)
% ENERGY_MATCHING_SVD_DENOISING  Same as sure_svd_denoising but with
%   energy-matching rank selection (Guo et al. Eq. 22).
%   Calls the shared pipeline with method='energy'.
    ...
    % Stage 1
    x0 = one_pass(noisy, sigma, ps, L, sw, step, 'energy');

    % Back projection
    y_tilde = x0 + delta * (noisy - x0);
    [H, W] = size(noisy);
    ytilde_x0_mse = sum((y_tilde(:) - x0(:)).^2) / (H*W);
    tau_new = gamma_bp * sqrt(max(sigma^2 - ytilde_x0_mse, 1));

    % Stage 2
    denoised = one_pass(y_tilde, tau_new, ps, L, sw, step, 'energy');
...
% All helper functions identical to sure_svd_denoising.m:
...
function est = one_pass(img, tau, ps, L, sw, step, method)
    ...
    for ci = 1:length(cols_ref)
        ...
        for ri = 1:length(rows_ref)
            row = rows_ref(ri);
            ref = all_patches(:, (col-1)*N_row + row);
            ...
            for p = 1:length(indc)
                ...
                est_acc(pr:pr+ps-1, pc:pc+ps-1) = est_acc(pr:pr+ps-1, pc:pc+ps-1) + w_j * patch_2d;
                weight_acc(pr:pr+ps-1, pc:pc+ps-1) = weight_acc(pr:pr+ps-1, pc:pc+ps-1) + w_j;
            end
        end
    end
